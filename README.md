...
```
Process spectrum sample-A.txt with lambda = 10000 (baseline parameter), window length = 7 and polynomial order = 4 (smoothing parameters) in the range from xmin = 600 to xmax = 800 <sup>-1</sup>, annotate peaks with intenisies equal or greater than t = 50 and save the PNG and DATA files (-spd).
